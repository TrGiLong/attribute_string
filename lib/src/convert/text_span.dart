...
extension _AttributeStringRootBuilder on AttributeStringTreeRoot {
  TextSpan build({TextStyle baseStyle}) {
    return TextSpan(children: this.paragraphs.map((p) => p.build(baseStyle: baseStyle)).expand((element) => element.children).toList());
  }
}

extension _AttributeStringParagraphBuilder on AttributeStringTreeParagraph {
  TextSpan build({TextStyle baseStyle}) {
    return TextSpan(children: this.nodes.map((node) => node.build(baseStyle: baseStyle)).toList());
  }
}
...
Map<String, StyleBuilder> _styles = {
  ...
  Attribute.Size: (value) => TextStyle(fontSize: double.tryParse(value)),
...
extension _AttributeStringNodeBuilder on AttributeStringTreeNode {
  TextSpan build({TextStyle baseStyle}) {
    var styles = this.styles.values.map((style) {
      StyleBuilder styleBuilder = _styles[style.key];
      if (styleBuilder != null) return styleBuilder(style.value);
    });
